/**
 * QR Code Renderer — fully local, zero-dependency QR code generator
 * for the Electron renderer process.
 *
 * Ported from src/main/qr-code.js — replaced Buffer.from() with TextEncoder
 * so the module runs in the browser context without Node.js APIs.
 *
 * Supports QR versions 1–10, ECC level M (medium).
 */
(function installQrCodeRenderer(global) {
  "use strict";
  ...
  // Browser-compatible UTF-8 encoding (replaces Node's Buffer.from)
  ...
  global.LocalAgentQrCodeRenderer = {
    renderQrPayload
  };
})(window);
